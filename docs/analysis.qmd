...
p_catfish_control <- emmeans_bb %>% filter(contrast == "control / catfish") %>% pull(p.value)
...
p_dark_light      <- emmeans_bb %>% filter(contrast == "light / dark")      %>% pull(p.value)
...
emmeans_plot <- ggplot(emm_all, aes(x = x, y = prob, ymin = asymp.LCL, ymax = asymp.UCL)) +
  ...
  facet_wrap(~ factor, scales = "free_x", labeller = as_labeller(str_to_title)) +
  scale_y_continuous("Predicted probability of refuge use", limits = c(0, 1)) +
  scale_x_discrete(NULL, labels = str_to_title) +
...
emm_treatment <- bind_rows(emm_treat_pred) %>%
  mutate(factor = factor(factor, levels = c("treatment")), x = factor(x, levels = c("catfish", "eel", "control")))

emmeans_treatment_plot <- ggplot(emm_treatment, aes(x = x, y = prob, ymin = asymp.LCL, ymax = asymp.UCL)) +
  geom_point(size = 3) +
  geom_errorbar(width = 0.2) +
  geom_text(data = cld_treatment, aes(x = x, y = asymp.UCL + 0.05, label = .group), inherit.aes = FALSE) +
  scale_y_continuous("Predicted probability of refuge use", limits = c(0, 1)) +
  scale_x_discrete(NULL, labels = str_to_title) +
  geom_hline(yintercept = 0.5, linetype = "dashed", colour = "grey50")

emmeans_treatment_plot
