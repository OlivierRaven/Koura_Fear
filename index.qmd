...
.i <- file.path(dirname(knitr::current_input(dir = TRUE)), "02_koura_fear", "images", "ch2_cover.jpg")
if (file.exists(.i)) cat(sprintf('<div class="ch-hero-wrap"><img class="ch-hero-img" src="%s" alt=""><div class="ch-hero-fade"></div><div class="ch-hero-text"><span class="ch-eyebrow">Chapter 2</span><span class="ch-hero-title">Native vs. Novel Threats</span></div></div>', knitr::image_uri(.i)))
```

# Chapter 2 Native vs. Novel Threats {.unnumbered .ch-page-heading}
:::
...
unlink(tmp_r)
out_dir <- file.path(chapter_dir, "outputs")
ch2_out_dir <- out_dir  # saved so appendices can find it after later chapters overwrite out_dir
...
local({
  dat <- read.csv(file.path(out_dir, "tbl-emmeans-stratified.csv"))
  col_nms <- c("Condition", "Contrast", "Odds ratio", "SE", "df", "Null", "z value", "p value")
  if (isTRUE(getOption("thesis.is.html"))) {
    DT::datatable(dat, colnames = col_nms, rownames = FALSE, filter = "top",
                  options = list(pageLength = 15, dom = 'frtip', scrollX = TRUE)) |>
      DT::formatRound(columns = seq_along(dat)[sapply(dat, is.numeric)], digits = 3)
  } else if (knitr::is_latex_output()) {
    knitr::kable(dat, col.names = col_nms, digits = 3, booktabs = TRUE) |>
