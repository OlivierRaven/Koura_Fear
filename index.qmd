...
# Always: purl analysis code and eval in parent env so inline R expressions work
tmp_r <- tempfile(fileext = ".R")
withr::with_dir(chapter_dir, {
  knitr::purl("analysis.qmd", output = tmp_r, quiet = TRUE, documentation = 0)
  eval(parse(file = tmp_r), envir = parent_knit_env)
})

# Only if PNGs missing: run full knit to generate figure files
if (!all(file.exists(file.path(out_dir, paste0(fig_labels, ".png"))))) {
  withr::with_dir(chapter_dir, {
    knitr::knit("analysis.qmd", output = tempfile(), quiet = TRUE, envir = parent_knit_env)
  })
